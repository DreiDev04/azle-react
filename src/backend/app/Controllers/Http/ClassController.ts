...
export default class ClassController {
  static async classes(request: Request, response: Response) {
    ...
    try {
      ...
      return response
        .status(200)
        .json({ message: "Classes found!", payload: classes }); // Return the found classes
    } catch (error) {
  ...
  static async create_class(request: Request, response: Response) {
    const { class_name, class_description, class_ownerId, class_membersIds } =
      request.body;
    try {
      ...
      const class_members = await User.findBy({
        user_id: In(class_membersIds || []),
      });
      if (
        class_membersIds &&
        class_members.length !== class_membersIds.length
      ) {
      ...
      return response
        .status(200)
        .json({ message: "Class created!", payload: classEntity });
    } catch (error) {
      return response
        .status(400)
        .json({ message: "Error in creating class: ", error: error });
    }
  }
  ...
  static async get_class(request: Request, response: Response) {
    ...
    try {
      ...
      return response
        .status(404)
        .json({ message: "Error in getting class: ", error });
    }
  }
  ...
  static async update_class(request: Request, response: Response) {
    ...
    try {
      ...
      return response
        .status(400)
        .json({ message: "Error updating class: ", error });
    }
  }
  ...
  static async delete_class(request: Request, response: Response) {
    ...
    try {
      ...
      return response
        .status(400)
        .json({ message: "Error creating deck: ", error });
    }
  }
}
